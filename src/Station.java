...
public class Station {
    ...
    public void startNextTask() {
        if (!taskQueue.isEmpty() && executingTasks.size() < capacity) {
            ...
            System.out.println("Task " + task.getType().getTaskTypeID() + " started at station " + stationID);
        }
    }

    public void completeTask(Task task) {
        executingTasks.remove(task);
        System.out.println("Task " + task.getType().getTaskTypeID() + " completed at station " + stationID);
    }
    ...
    public void addTaskTypeSpeed(TaskType taskType, double speed, double variation) {
        taskSpeeds.put(taskType.getTaskTypeID(), speed);
        taskSpeedVariations.put(taskType.getTaskTypeID(), variation);
    }

    public void startTask(Task task) {
        double speed = getTaskSpeed(task.getType().getTaskTypeID());
        double variation = getTaskSpeedVariation(task.getType().getTaskTypeID());
    ...
    public boolean canHandleTask(Task task) {
        return supportsTaskType(task.getType().getTaskTypeID()) && executingTasks.size() < capacity;
    }
